...
class Trainer:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    # Avoid unnecessary DDP synchronization since there will be no backward pass on this example.
                    with model.no_sync():
                        tr_loss_step = self.training_step_zjs(model, inputs)
                else:
                    tr_loss_step = self.training_step_zjs(model, inputs)
    ...
    def training_step_zjs(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        """
        Perform a training step on a batch of inputs.
        ...
        if self.use_amp:
            ...
            rs_index = torch.tensor([1, 2, 3, 4]).to(model.device)
            inputs_rs = torch.index_select(inputs.data['input_ids'], dim = 1, index = rs_index)
            inputs.data['input_ids'] = torch.cat((inputs.data['input_ids'][:,:1], inputs.data['input_ids'][:, 5:]), dim = 1)
            inputs.data['attention_mask'] = torch.cat((inputs.data['attention_mask'][:, :1], inputs.data['attention_mask'][:, 5:]),
                                                 dim = 1)
            loss = self.compute_loss_zjs(model, inputs, inputs_rs = inputs_rs)
    ...
    def compute_loss_zjs(self, model, inputs, inputs_rs = None, return_outputs=False):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        outputs = model.forward_zjs(**inputs, input_rs = inputs_rs)
        # Save past state if it exists
        # TODO: this needs to be fixed and made cleaner later.
        if self.args.past_index >= 0:
            self._past = outputs[self.args.past_index]
